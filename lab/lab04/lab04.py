...
def reverse_recursive(lst):
    ...
    if length == 0:
        return []
    r_lst = []
    r_lst.append(lst[length-1])
    return r_lst + reverse_recursive(lst[0:length-1])
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x_square = (get_lat(city_a)-get_lat(city_b)) ** 2
    y_square = (get_lon(city_a)-get_lon(city_b)) ** 2
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    anonymous_city = make_city('anonymous_city',lat,lon)
    d_a = distance(anonymous_city,city_a)
    d_b = distance(anonymous_city,city_b)

    return get_name(city_a) if d_a <= d_b else get_name(city_b)
