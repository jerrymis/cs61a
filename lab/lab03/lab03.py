...
def pingpong(n):
    ...
    def pingpong_helper(value, index, direction):
        ...
        if index % 8 == 0 or num_eights(index) > 0:
            if direction == 1:
                return pingpong_helper(value-1,index+1,0)
            else:
                return pingpong_helper(value+1,index+1,1)
        else:
            if direction == 1:
                return pingpong_helper(value+1,index+1,direction)
            else:
                return pingpong_helper(value-1,index+1,direction)
